 ...
 */
package com.bitactor.framework.core.utils.collection;

import com.bitactor.framework.core.utils.lang.ArrayUtils;
...
public class CollectionUtils {
    ...
    public static <K, V> List<K> getKeys(Map<K, V> map) {
        return map.keySet().stream().filter(Objects::nonNull).collect(Collectors.toList());
    }
    /**
    ...
    /**
     * 将 source 的数据克隆到 target，被克隆target 将先要被清空
     *
     * @param target 克隆的目标
     * @param source 克隆的资源
     */
    @SuppressWarnings({"unchecked", "rawtypes", "deprecation"})
    public static void cloneMap(Map target, Map source) {
     ...
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    public static void cloneCollection(Collection target, Collection source) {
        if (target == null || source == null) {
            return;
        }
        target.clear();
