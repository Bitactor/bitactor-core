...
public class ClassHelper {
     ...
     * as value, for example: Integer.class -> int.class.
     */
    private static final Set<Class<?>> primitiveWrapperTypeSet = new HashSet<Class<?>>(8);
    ...
    public static boolean isPrimitiveType(Class<?> cl) {
        return primitiveWrapperTypeSet.contains(cl);
    }
     ...
     * @return
     */
    public static boolean isArrayType(Class<?> cl) {
        int isPrimitiveArray = cl.getName().indexOf(INTERNAL_ARRAY_PREFIX);
        if (isPrimitiveArray != -1 && cl.getName().endsWith(";")) {
            return true;
        }
        return false;
    }
     ...
     * @return
     */
    public static boolean isPrimitiveArrayType(Class<?> cl) {
        try {
            String name = cl.getName();
            int isPrimitiveArray = name.indexOf(INTERNAL_ARRAY_PREFIX);
            if (isPrimitiveArray != -1 && name.endsWith(";")) {
                int internalArrayMarker = name.indexOf("[L");
                String elementClassName = null;
                if (internalArrayMarker == 0) {
                    elementClassName = name
                            .substring("[L".length(), name.length() - 1);
                } else if (name.startsWith("[")) {
                    elementClassName = name.substring(1);
                }
                if (isPrimitiveType(ClassUtils.forName(elementClassName))) {
                    return true;
                }
            }
    ...
    /**
     * 原始 Object[]复制
     * 将 source 的数据克隆到 target
     *
     * @param target 克隆的目标
     * @param source 克隆的资源
     */
    @SuppressWarnings({"unchecked", "rawtypes", "deprecation"})
    public static void cloneArray(Object[] target, Object[] source) {
        if (target == null || source == null) {
            return;
        }
        int length = Math.min(target.length, source.length);
        if (isPrimitiveArrayType(target.getClass())) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                CglibBeanCopierUtils.copyProperties(target[i], source[i]);
            }
        }
    }
     ...
     *
     * @param field
     * @return 如果不是泛型类型字段则返回null
     */
    public static Type[] getGenericType(Field field) {
        if (!(field.getGenericType() instanceof ParameterizedType)) {
            System.out.println(" not genericType");
            return null;
        }
        ParameterizedType listGenericType = (ParameterizedType) field.getGenericType();
        return listGenericType.getActualTypeArguments();
    }
